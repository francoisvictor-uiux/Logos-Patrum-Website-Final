...
void main() {
  ...
  // Apply a subtle parallax offset based on uMouse
  // uMouse is [0,1], center is 0.5
...
uniform vec3 uPalette[20];
uniform float uPaletteA[20];
...
void main() {
  ...
  if (cnt <= 1) {
    ...
    // 16 colors: Smoothly interpolate between first 8 (blues) and last 8 (oranges)
    int cnt2 = 8;
    float scaled = g2 * float(cnt2 - 1);
    int i0 = int(floor(scaled));
    i0 = clamp(i0, 0, cnt2 - 2);
    float f = scaled - float(i0);
    
    vec3 colA = mix(uPalette[i0], uPalette[i0 + 1], f);
    float alphaA = mix(uPaletteA[i0], uPaletteA[i0 + 1], f);
    
    vec3 colB = mix(uPalette[i0 + cnt2], uPalette[i0 + cnt2 + 1], f);
    float alphaB = mix(uPaletteA[i0 + cnt2], uPaletteA[i0 + cnt2 + 1], f);
    
    // Oscillate between 0 and 1 over time (uTime is in seconds)
    float mixFactor = sin(uTime * 0.2) * 0.5 + 0.5;
    
    dotCol = mix(colA, colB, mixFactor);
    dotOpacity = mix(alphaA, alphaB, mixFactor);
  } else {
...
export default function DottedBackground({
    ...
    useEffect(() => {
        ...
        const mouse = { x: -2.0, y: -2.0, targetX: -2.0, targetY: -2.0 };
        ...
        const update = (time: number) => {
            ...
            if (perlinProgramRef.current) {
                perlinProgramRef.current.uniforms.uTime.value = time * 0.001;
            }
            if (dotProgramRef.current) {
                dotProgramRef.current.uniforms.uTime.value = time * 0.001;
            }

            // Mouse lerp
            mouse.x += (mouse.targetX - mouse.x) * 0.1;
            mouse.y += (mouse.targetY - mouse.y) * 0.1;
            if (dotProgram.uniforms.uMouse) {
                dotProgram.uniforms.uMouse.value = [mouse.x, mouse.y];
            }
            if (perlinProgram.uniforms.uMouse) {
                perlinProgram.uniforms.uMouse.value = [mouse.x, mouse.y];
            }
