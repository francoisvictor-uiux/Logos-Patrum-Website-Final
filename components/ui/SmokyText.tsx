...
type Phase = "hidden" | "appearing" | "visible" | "disappearing"
...
export default function SmokyText(__props: any) {
    ...
    useEffect(() => {
        if (!isActive) {
            if (phase !== "hidden" && phase !== "disappearing") {
                clear()
                setPhase("disappearing")
                later(() => {
                    setPhase("hidden")
                }, appearT.duration * 1000 + 200)
            }
        }
    }, [isActive, phase, clear, later, appearT.duration])
                                ...
                                if (phase === "disappearing") {
                                    ...
                                    return (
                                        <span
                                            ...
                                            style={{
                                                ...base,
                                                animation: `${anim} ${charDur}s ${delay}s ${appearT.timing} reverse both`,
                                            }}
                                        >
                                            {c.char}
                                        </span>
