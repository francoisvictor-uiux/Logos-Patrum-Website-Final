...
/**
 * DigitalRain — columns of glyphs falling behind the page.
 *
 * Each column runs one or more streams. A stream is a head glyph with a tail
 * behind it; it falls at its own rate, and most streams burn out somewhere past
 * three quarters of the way down while a minority cross the whole field. A
 * column releases its next stream once the current one is far enough along, so
 * the rain never gaps.
 *
 * Adapted from the Originkit Framer component, with three changes for this
 * codebase:
 *
 *   1. The preset that came with it — the John 1:1 prologue as the glyph
 *      source, orange-300 tails, 9px type — is folded into DEFAULTS rather than
 *      wrapped around the export, so the props are the API.
 *   2. Glyphs are culled against the viewport rather than the rotated field's
 *      diagonal. The original tested every glyph against ±span/2, which is the
 *      whole tilted square: at this glyph size that left roughly half the draw
 *      calls painting outside the box every frame.
 *   3. A reader who has asked for reduced motion gets one still frame — the
 *      field as it seeds, already mid-flight — instead of the loop.
 *
 * The canvas paints on transparency, so whatever is behind it shows through.
 */

const DEFAULTS = {
  headColor: "#FFFFFF",
  /* orange-300 — the same step the journey spine uses. */
  trailColor: "#FFC08F",
  glyphSize: 9,
  speed: 7,
  angle: 0,
  ...
  shuffle: true,
  shuffleGlyphs:
    "Ἐν ἀρχῇ ἦν ὁ λόγος, καὶ ὁ λόγος ἦν πρὸς τὸν θεόν, καὶ θεὸς ἦν ὁ λόγος.οὗτος ἦν ἐν ἀρχῇ πρὸς τὸν θεόν. πάντα δι' αὐτοῦ ἐγένετο, καὶ χωρὶς αὐτοῦ ἐγένετο οὐδὲ ἕν. ὃ γέγονεν ἐν αὐτῷ ζωὴ ἦν, καὶ ἡ ζωὴ ἦν τὸ φῶς τῶνἀνθρώπων·καὶ τὸ φῶς ἐν τῇ σκοτίᾳ φαίνει, καὶ ἡ σκοτία αὐτὸ οὐ κατέλαβεν.",
};
...
export default function DigitalRain(props: DigitalRainProps) {
  ...
  useEffect(() => {
    ...
    function layout() {
      ...
      // Under reduced motion nothing will ask for another frame, so the seeded
      // field is the picture — paint it here or the canvas stays empty.
      if (still) draw(0);
    }
    ...
    function draw(dt: number) {
      ...
      // Half-extents of the real box, in the rotated frame's units, with a
      // glyph of slack so nothing pops at the edge.
      const limitX = w / 2 + glyphSize;
      const limitY = h / 2 + glyphSize;
      ...
      for (let i = 0; i < cols; i++) {
        ...
        for (const stream of column.streams) {
          ...
          for (let j = 0; j < tailLength; j++) {
            ...
            // Rotate the glyph back out to screen space and test it against
            // the box we can actually see. At angle 0 this is a plain bounds
            // check; tilted, it is the same check in the tilted frame.
            if (Math.abs(x * cos - y * sin) > limitX) continue;
            if (Math.abs(x * sin + y * cos) > limitY) continue;
  ...
  return (
    <div ref={wrapRef} style={{ ...style, position: "relative", overflow: "hidden" }}>
